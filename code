...
def print_main_screen():
    ...
    title = [0] * 11
    title[10] = small_titles.render(f'выберите раздел соответствующей цифрой на клавиатуре', 1, pygame.Color('red'))
    ...
    title[6] = small_titles.render(f'6. Уровень 6 (custom)', 1, pygame.Color('red'))
    title[7] = small_titles.render(f'7. Таблица рекордов', 1, pygame.Color('red'))
    title[8] = small_titles.render(f'8. Настройка цвета змеи', 1, pygame.Color('red'))
    title[9] = small_titles.render(f'9. Ввод имени для таблицы рекордов', 1, pygame.Color('red'))
...
def set_walls_custom():
    walls = []
    initiall_point_wo_walls(15)
    draw_screen()
    pygame.display.flip()
    running = True
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    cell_x = mouse_pos[0] // Consts.SIZE_1
                    cell_y = mouse_pos[1] // Consts.SIZE_1
                    walls.append((Consts.min_coord + (cell_x - 5) * Consts.SIZE_1, Consts.min_coord + (cell_y - 5) * Consts.SIZE_1))
    return walls

def level_6():
    walls = set_walls_custom()
    FPS_6 = 15
    initiall_point_wo_walls(FPS_6)
    is_apple_and_snake_in_wall(walls)
    while True:
        draw_screen()
        draw_walls(walls)
